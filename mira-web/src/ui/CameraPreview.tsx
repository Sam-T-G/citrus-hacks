...
import type { VisionPipeline, RawVisionFrame } from '../vision/VisionPipeline';
import { CUE_NAMES, STATE_WEIGHTS, SAFETY_CUES } from '../vision/vision-constants';
import { useSession } from '../session/SessionContext';
import type { ArduinoCommand, EngineState, FaceType } from '../types';

interface Props {
  camera:      CameraCapture | null;
  vision?:     VisionPipeline | null;
  lastCmd:     ArduinoCommand | null;
  active:      boolean;
  engineState: EngineState;
}

const LCD_ROWS: Record<FaceType, [string, string]> = {
  happy: ['(  ^  )  (  ^  )', '~~~~~~~~~~~~~~~~'],
  plain: ['(  B  )  (  B  )', '----------------'],
  wink:  ['(  ^  )  (  -  )', '----------------'],
  tears: ['(  T  )  (  T  )', '   ||       ||  '],
...
const STANDBY: [string, string] = ['(  B  )  (  B  )', '- - - - - - - - '];
...
// ── Owl LCD ──────────────────────────────────────────────────────────────────
function OwlLCD({ face, waving, large }: { face: FaceType | null; waving: boolean; large?: boolean }) {
  const [row0, row1] = face ? LCD_ROWS[face] : STANDBY;
  const cls = `owl-lcd${!face ? ' owl-lcd--none' : ''}${large ? ' owl-lcd--large' : ''}`;
  return (
    <div className={cls}>
      <div className="owl-lcd__face">{row0}</div>
      <div className="owl-lcd__row2">{row1}</div>
      <div className="owl-lcd__name">{face ?? 'standby'}{waving ? ' · waving' : ''}</div>
    </div>
  );
}
...
function CollapseIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round">
      <path d="M5 1v4H1M9 5V1h4M9 13v-4h4M1 9h4v4" />
    </svg>
  );
}

// ── CV Overlay canvas — draws MediaPipe landmarks over video ─────────────────
function CVOverlay({ canvasRef }: { canvasRef: React.RefObject<HTMLCanvasElement> }) {
  return (
    <canvas
      ref={canvasRef}
      style={{
...
interface CueEntry { ts: number; cue: string; conf: number }
...
function BehaviorPanel({ cues, alive }: { cues: CueEntry[]; alive: boolean }) {
  ...
  const sectionLabel = (label: string, dot?: string) => (
    <div style={{
      ...
      {dot !== undefined && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <span style={{ color: alive ? 'rgba(74,222,128,0.6)' : 'rgba(250,247,242,0.25)' }}>{dot}</span>
        </div>
      )}
    </div>
  );
  ...
  const empty = (msg: string) => (
    <div style={{ color: 'rgba(250,247,242,0.28)', fontSize: 10.5 }}>{msg}</div>
  );
  ...
  return (
    <div style={panelStyle}>
      {/* ── Gemini observations ── */}
      <div>
        {sectionLabel('Gemini · Visual Read')}
        {recentObs.length === 0
          ...
          : recentObs.map((o, i) => {
              ...
              return (
                <div key={i} style={{ marginBottom: 7, paddingBottom: 7, borderBottom: '0.5px solid rgba(250,247,242,0.05)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 3 }}>
                    ...
                    <span style={{ fontSize: 9, color: 'rgba(250,247,242,0.3)' }}>{time}</span>
                  </div>
                  <div style={{ fontSize: 10.5, color: 'rgba(250,247,242,0.75)', lineHeight: 1.45 }}>{o.description}</div>
                </div>
              );
            })
        }
      ...
      {/* ── MediaPipe gesture cues ── */}
      <div>
        {sectionLabel('MediaPipe · Gestures', alive ? 'live' : 'idle')}
        {recentCues.length === 0
          ...
          : recentCues.map((e, i) => {
              ...
              return (
                <div key={i} style={{ marginBottom: 7, paddingBottom: 7, borderBottom: '0.5px solid rgba(250,247,242,0.05)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 3 }}>
                    <span style={{ color: isSafe ? 'rgba(251,146,60,0.95)' : 'rgba(250,247,242,0.9)', fontWeight: 600 }}>
                      {name}{isSafe ? ' ⚠' : ''}
                    </span>
                    <span style={{ fontSize: 9, color: 'rgba(250,247,242,0.3)' }}>
                      {time} · {(e.conf * 100).toFixed(0)}%
                    </span>
                  </div>
                  {Object.keys(weights).length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      {Object.entries(weights).map(([state, w]) => (
...
function CameraModal({
  camera, vision, lastCmd, active, engineState, onClose,
}: Props & { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const onFrame = (frame: RawVisionFrame) => {
      setPipeAlive(true);
      const canvas = canvasRef.current;
      const video  = videoRef.current;
      ...
      <div style={{
        ...
        <div style={{
          ...
          >
            {active
              ...
              : (
                <div style={{
                  ...
                  <span style={{ fontFamily: 'var(--csans)', fontSize: 14, letterSpacing: '0.06em' }}>Camera inactive</span>
                </div>
              )
            }
            {/* State ring */}
            ...
            {/* CV canvas overlay */}
            {showCV && <CVOverlay canvasRef={canvasRef} />}
          </div>

          {/* Debug panel — only when showCV */}
          {showCV && (
            <div style={{ width: 280, flexShrink: 0 }}>
              <BehaviorPanel cues={cues} alive={pipeAlive} />
            </div>
          )}
        </div>
        ...
        <div style={{ width: '100%' }}>
          ...
          <OwlLCD face={face} waving={waving} large />
        </div>
      </div>
    </div>
  );
...
export function CameraPreview({ camera, vision, lastCmd, active, engineState }: Props) {
  const videoRef  = useRef<HTMLVideoElement>(null);
    ...
    <>
      <div className="camera-panel">
        <div className="camera-wrap">
          ...
          <div className={`camera-ring camera-ring--${active ? engineState : 'idle'}`} />
        ...
        <div className="owl-panel">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div className="owl-panel__label">Owl LCD · 16×2</div>
            <button onClick={() => setExpanded(true)} className="camera-expand-btn-inline">
              <ExpandIcon />
            </button>
          </div>
          <OwlLCD face={face} waving={waving} />
        </div>
      </div>
