...
export class GestureDetector {
  ...
  private _detect(
    ...
    if (faceLandmarks && faceLandmarks.length > 400) {
      ...
      if (this.faceWidthHistory.length > this.HISTORY_LIMIT) this.faceWidthHistory.shift();
    ...
    if (this.headYHistory.length === this.HISTORY_LIMIT) {
      ...
      if (maxY - minY > 0.028) return this._emit('nod_yes', 0.9);

      const minX = Math.min(...this.headXHistory), maxX = Math.max(...this.headXHistory);
      if (maxX - minX > 0.038) return this._emit('shake_no', 0.9);

      // Slower left-right scan with smaller amplitude → disoriented looking around
      if (maxX - minX > 0.016) return this._emit('looking_around_confused', 0.78);
    }

    // ── Priority 1: Blendshapes (most reliable expression read) ──────────────
    if (blendshapes.length > 0) {
      const raw = (name: string) =>
        blendshapes.find(b => b.categoryName === name || b.categoryName.includes(name))?.score ?? 0;
      const rel = (name: string) => Math.max(0, raw(name) - (this.baseline[name] ?? 0));
      ...
      if (blink > 0.40)        return this._emit('blinking',       blink);
      if (smile > 0.08)        return this._emit('smiling',        smile);
      if (frown > 0.06)        return this._emit('frowning',       frown);
      if (browsRaised > 0.07)  return this._emit('brows_raised',   browsRaised);
      if (browsDown > 0.08)    return this._emit('brows_furrowed', browsDown);
      if (gazeUp > 0.22)       return this._emit('looking_up',     gazeUp);
      if (gazeDown > 0.22)     return this._emit('looking_down',   gazeDown);
    }
    ...
    if (handLandmarks?.length) {
      for (const hand of handLandmarks) {
        ...
        if (isFist) {
          if (thumbTip.y < wrist.y - 0.08) return this._emit('thumbs_up',   1.0);
          if (thumbTip.y > wrist.y + 0.08) return this._emit('thumbs_down', 1.0);
        }
      }
    }
    ...
    if (poseLandmarks && poseLandmarks.length >= 33) {
      ...
      if (distLR < 0.12 && distRL < 0.12) return this._emit('crossed_arms', 0.9);

      const shoulderZ = (lShoulder.z + rShoulder.z) / 2;
      if (shoulderZ < -0.35) return this._emit('leaning_forward', Math.abs(shoulderZ));
      if (shoulderZ > 0.18)  return this._emit('leaning_back',    shoulderZ);
      ...
      const shoulderAvgY = (lShoulder.y + rShoulder.y) / 2;
      if (shoulderAvgY - nose.y > 0.22) return this._emit('slouching', 0.8);
    }
    ...
    if (faceLandmarks && faceLandmarks.length > 400) {
      ...
      if (noseOffset > 0.40 && noseOffset < 0.60) return this._emit('facing_you', 1.0);
      if (noseOffset < 0.30 || noseOffset > 0.70)  return this._emit('looking_away', 1.0);
      ...
      if (browRatio > 0.055)  return this._emit('brows_furrowed', Math.min(1.0, browRatio * 8));
      if (browRatio < -0.07)  return this._emit('brows_raised',   Math.min(1.0, Math.abs(browRatio) * 4));
      ...
      if (smileScore > 0.10)  return this._emit('smiling',  Math.min(1.0, smileScore * 3.5));
      if (smileScore < -0.04) return this._emit('frowning', Math.min(1.0, Math.abs(smileScore) * 5));
    }
    ...
    if (poseLandmarks && poseLandmarks.length >= 17) {
      ...
      if (rWrist && rElbow && rWrist.y < rElbow.y) {
        ...
        if (this.wristXHistory.length === this.HISTORY_LIMIT) {
          ...
          if (maxX - minX > 0.07) return this._emit('wave_detected', 0.82);
        }
      } else {
        this.wristXHistory = [];
      }
  ...
  getInterpretedState(): { state: SocialState; scores: Record<SocialState, number> } {
    ...
    recent.forEach(ev => {
      ...
      if (w) Object.entries(w).forEach(([s, wt]) => {
        scores[s as SocialState] += (wt ?? 0) * ev.confidence;
      });
    });

    let topState: SocialState = 'neutral';
    let maxScore = 1.5; // lower threshold so states register sooner
    (Object.entries(scores) as [SocialState, number][]).forEach(([s, sc]) => {
      if (sc > maxScore) { maxScore = sc; topState = s; }
    });

    return { state: topState, scores };
  }

  getRecentCues(windowMs = 8000): CueEvent[] {
    const now = Date.now();
  ...
  private _emit(cue: string, score = 1.0): { cue: string; score: number } | null {
    const now = Date.now();
